...
import MarketingCaseDetail from "./pages/MarketingCaseDetail";
...
const App: React.FC = () => {
  ...
  const handleLogin = (newUser: any) => {
    // JWT token decode və ya backend-dən əlavə info almaq lazımdır
    // Sadə halda, user obyektinə rol və id əlavə et
    let role: UserRole = UserRole.DOCTOR;
    if (newUser && newUser.is_marketing) role = UserRole.MARKETING;
    setUser({
      ...newUser,
      role,
      id: newUser.id || newUser.user_id || newUser.username || newUser.email,
      name: newUser.name || newUser.full_name || newUser.first_name || newUser.email,
    });
  };
          ...
          <>
            ...
            <main className="flex-1 overflow-x-hidden p-6 md:p-12 lg:p-16 pb-32 md:pb-16">
              <Routes>
                ...
                <Route path="/new-case" element={user.role === UserRole.DOCTOR ? <NewCasePage user={user} onAddCase={addCase} /> : <Navigate to="/dashboard" replace />} />
                <Route path="/my-cases" element={user.role === UserRole.DOCTOR ? <MyCasesPage cases={cases.filter(c => c.doctorId === user.id)} /> : <Navigate to="/dashboard" replace />} />
                <Route path="/stats" element={<StatsPage cases={cases} user={user} />} />
                <Route path="/badges" element={user.role === UserRole.DOCTOR ? <BadgesPage user={user} /> : <Navigate to="/dashboard" replace />} />
